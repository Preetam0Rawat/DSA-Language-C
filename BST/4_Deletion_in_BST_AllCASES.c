...
struct tree * delete_node(struct tree *rt, int val)
{
    if(rt == NULL)
       return NULL;

    else if( val < rt->data)
         rt->left = delete_node(rt->left, val);  
    
    else if(val > rt->data)
         rt->right =  delete_node(rt->right, val); 
    ...
    else    // Found the value to be deleted
    {
         if(rt->left == NULL && rt->right == NULL)          //CASE 1: Data found at leaf node.
          ...
          else                                              //CASE 3: Data found at node with 2 children. 
          {
             ...
             rt->data = temp->data;
             rt->right = delete_node(rt->right, temp->data);       //Deleting the duplicte node
